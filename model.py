...
from utubextract import get_transcript_of_yt_video
from translate import g_translate
from preTrained import *
from write import makeTextFile
...
def nlp_model(v_id):
    ...
    if (transcript == '0'):
        ...
        output=work(english_summary,900,6)

        hindi_translated_summary = g_translate(output, 'hi')

        tamil_translated_summary = g_translate(output, 'ta')

        makeTextFile("English", output)
        makeTextFile("Hindi", hindi_translated_summary)
        makeTextFile("Tamil", tamil_translated_summary)

        return original_text_length, final_summary_length, output, hindi_translated_summary, tamil_translated_summary
